...
| PILOT-R-006 | Workflow automation expands autonomy too early | High | A3/A4 blocked; no networked connectors or model providers; dry-run worker has no network or effects | Mitigated in experiment |
...
| PILOT-R-024 | Two workers duplicate one delivery | High | Serialized claim transition, exact Connector Instance, bounded lease and fencing claim ID | Mitigated for one local process / distributed residual open |
...
| PILOT-R-028 | Dry-run result is mistaken for connector validation | High | Network/effects false in code, status and evidence; separate read-only sandbox gate required | Open interpretation risk |
